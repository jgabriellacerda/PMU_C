...
file = sprintf('SinaisFaseMagCM/fase_cm_%s0.txt',sTest);
fase_cm = load(file);
fase_cm = fase_cm(1:end);

file = sprintf('SinaisFaseMagCM/mag_cm_%s0.txt',sTest);
mag_cm = load(file);
mag_cm = mag_cm(1:end);
...
[freq_cm,legs,delay_SG] = FreqSG(unwrap(fase_cm),fs);
corr_cm = (coef_a*(-freq_cm(1,1:length(fase_cm)))' + coef_b);
...
figure;
plot(unwrap(fase_cm(floor(delay_N+1):end)),'-o','LineWidth',3);
hold on
...
TVE_cm = TVE(fase_cm, mag_cm, fasor, fs, N);
...
overshoot = (max(fase_cm(degrau+floor(delay_N)+1:end))/fase_ref(degrau+1) - 1)*100;
